...
-   **An unknown download size counts as 0%.** That is the usual shape of an item stuck on "Downloading Metadata", so `maxProgress: 0` also covers the [metadata flow](#how-it-works) when `countDownloadingMetadataAsStalled` is enabled.
