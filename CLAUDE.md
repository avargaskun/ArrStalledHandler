...
2. **Poll loop**: for each `cfg.arr_apps` entry, `handle_stalled_downloads(cfg, app, qbit)` (queue items with `status=warning` and error `"the download is stalled with no connections"`) and `detect_stuck_metadata_downloads(cfg, app, qbit)` (`status=queued`, error `"qbittorrent is downloading metadata"`, gated by `cfg.count_metadata_as_stalled`). Both filters are case-insensitive. The flows differ only in query params and error filter; per-item logic is shared in `_process_queue_item`. Each handler returns the set of queue-item IDs it observed in a stalled state, or `None` when the queue read failed (the disabled-metadata path returns `set()`, not `None` — "this flow contributes nothing" is a complete statement). `main()` then calls `reconcile_tracked_downloads(app.name, stalled_seen | metadata_seen, ...)` once per app. Reconciling over the **union** is load-bearing: the two handlers read disjoint queue slices but share one table, so a handler pruning rows missing from its own results would delete the other flow's rows every cycle.
3. **Policy matching** (`match_watcher`): watchers are walked in order, first match wins, re-matched every cycle. An untagged watcher matches everything; tagged watchers require a configured downloader and a qBittorrent queue item. Whether an item is qBittorrent is decided against the *arr's `downloadclient` list, matched on `implementation` rather than the user-chosen client name — `resolve_qbittorrent_clients` fetches it once per handler call, only when some watcher declares tags, and returns the `LOOKUP_FAILED` sentinel when the call fails — `None` from that function means only "no lookup was needed", so `qbittorrent_client_names` returns the sentinel too rather than a `None` the caller could confuse with it. An instance with no qBittorrent client warns once per name (`_MISSING_CLIENT_WARNED`, re-armed when one reappears) since that is static; a failed lookup warns every cycle because each one means items were skipped — items that reach a tagged watcher then yield `SKIP_ITEM`, mirroring the failed-tag-lookup rule rather than falling through to a destructive default. Tags are fetched at most once per item, case-insensitively OR-matched. A failed tag lookup returns the `SKIP_ITEM` sentinel — the item is skipped this cycle rather than falling through to a destructive default. `load_config` guarantees a catch-all watcher exists.
4. **Timeout tracking (SQLite)**: first sighting inserts `(download_id, first_detected, arr_service)`; action is taken once elapsed time exceeds *the matched watcher's* `stalled_timeout`, then the row is deleted. `arr_service` is `ArrApp.name` — env mode synthesizes `Radarr0`/`Sonarr1`/`Lidarr0`/`Readarr0`, YAML mode uses the user's `name`, so renaming resets timers once. `prune_orphaned_services` clears rows for names no longer configured. An `IGNORE` match does no DB work at all, leaving any pre-existing row intact. `reconcile_tracked_downloads(arr_service, seen_ids, db_file)` runs every cycle and deletes every tracked row absent from `seen_ids`, so `first_detected` measures a *continuous* stall streak rather than time since the first ever sighting; it deletes with `executemany` because the stale set is unbounded and an `IN (...)` list past 32766 parameters would raise and kill the poll loop. The seen set is collected inside the error-message filter **before** watcher matching, so `IGNORE`- and `SKIP_ITEM`-resolved items still count as observed and keep their rows. Reconciliation is suppressed entirely for an app when either handler reports an incomplete view, so an *arr outage can never wipe its timers.
...
API versions differ by service: Radarr/Sonarr use `v3`, Lidarr/Readarr use `v1` (`ArrApp.api_version`). All queue reads go through `query_api_paginated`, which pages until `totalRecords` is reached. Its return contract mirrors `QbitClient.get_tags`: `None` = the read failed (any page errored, was malformed, or the API delivered fewer records than it reported), `[]` = the endpoint is genuinely empty, otherwise the full record list. A partial view must never be mistaken for a complete one, because reconciliation prunes against it.
